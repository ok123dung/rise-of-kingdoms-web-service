...
export function parseVNPayTimestamp(vnpayDate: string | undefined): number {
  ...
  try {
    ...
    const year = parseInt(vnpayDate.substring(0, 4), 10)
    const month = parseInt(vnpayDate.substring(4, 6), 10) - 1 // Month is 0-indexed
    const day = parseInt(vnpayDate.substring(6, 8), 10)
    const hour = parseInt(vnpayDate.substring(8, 10), 10)
    const minute = parseInt(vnpayDate.substring(10, 12), 10)
    const second = parseInt(vnpayDate.substring(12, 14), 10)
