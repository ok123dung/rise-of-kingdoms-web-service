  ...
  try {
    ...
    if (dbError instanceof Error) {
      if (dbError.message.includes('P1001')) {
        ...
        errorInfo.message = 'Database server reached timeout. This usually happens with serverless databases that need to wake up.'
        errorInfo.hint = 'Try again in a few seconds or consider using a different database provider.'
        ...
        errorInfo.hint = 'Run "prisma db push" or "prisma migrate deploy" to create the database schema.'
      }
    ...
    errorInfo.help_url = 'https://github.com/ok123dung/rok-services/blob/main/docs/VERCEL_DATABASE_SETUP.md'
