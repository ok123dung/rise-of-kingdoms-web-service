        ...
        <div className="rounded-2xl bg-white p-6 shadow-lg ring-1 ring-gray-900/5">
            ...
            <div className="mt-4 space-y-3">
                {methods.map((method) => (
                    <div
                        ...
                        className={`relative flex cursor-pointer items-center rounded-xl border p-4 transition-all ${selectedMethod === method.id
                                ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500'
                                : 'border-gray-200 hover:border-amber-200 hover:bg-gray-50'
                            }`}
                    >
                        <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${selectedMethod === method.id ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-500'
                            }`}>
                            <method.icon className="h-6 w-6" />
                        </div>
                        <div className="ml-4 flex-1">
                            <p className={`font-medium ${selectedMethod === method.id ? 'text-amber-900' : 'text-gray-900'}`}>
                                {method.name}
                            </p>
            ...
            <div className="mt-6 rounded-xl bg-gray-50 p-4 text-center">
                ...
                <button
                    ...
                    className="w-full rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 px-4 py-3 text-center font-semibold text-white shadow-lg shadow-amber-500/20 transition-all hover:-translate-y-0.5 hover:shadow-amber-500/30 disabled:opacity-70 disabled:cursor-not-allowed"
                >
                    {isProcessing ? (
