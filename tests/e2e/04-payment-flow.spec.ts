...
test.describe('Payment Flow Tests', () => {
  ...
  test('should select ZaloPay payment method', async ({ page }) => {
    ...
    const currentUrl = page.url()
    const hasAppLink = await page.locator('text=/Open ZaloPay App|Mở ứng dụng ZaloPay/i').isVisible()
  ...
  test('should validate payment before proceeding', async ({ page }) => {
    ...
    await expect(page.locator('text=/select payment method|chọn phương thức thanh toán/i')).toBeVisible()
  })
  ...
  test('should support payment cancellation', async ({ page }) => {
    ...
    const cancelBtn = page.locator('[data-testid="cancel-payment"], text=/cancel|hủy|back|quay lại/i')
