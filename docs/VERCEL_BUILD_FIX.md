...
The error shows: `Authentication failed against database server, the provided database credentials for 'postgres' are not valid`
...
API routes are using `headers()` which makes them dynamic, but Next.js is trying to render them statically.
...
Google Fonts requests are failing during build. This is likely due to network restrictions during build.
