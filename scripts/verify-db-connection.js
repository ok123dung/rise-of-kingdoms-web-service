...
async function testPooledConnection() {
    ...
    const pooledUrl = 'postgresql://postgres.inondhimzqiguvdhyjng:Dungvnn001*@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres?pgbouncer=true&connection_limit=1';
...
async function testDirectConnection() {
    ...
    const directUrl = 'postgresql://postgres:Dungvnn001*@db.inondhimzqiguvdhyjng.supabase.co:5432/postgres?sslmode=require';
    ...
    try {
        ...
        if (err.message.includes('certificate')) {
            console.error('💡 Hint: SSL certificate issue detected. Make sure ssl.rejectUnauthorized is set correctly.');
        }
        return false;
    }
}
...
async function verifySchema() {
    ...
    try {
        ...
        const expectedTables = ['users', 'services', 'service_tiers', 'bookings', 'payments', 'leads', 'staff'];
        const existingTables = tables.rows.map(r => r.tablename);
        const missingTables = expectedTables.filter(t => !existingTables.includes(t));

        console.log('📋 Total tables found:', tables.rows.length);
        console.log('✅ Expected tables present:', expectedTables.filter(t => existingTables.includes(t)).length + '/' + expectedTables.length);
